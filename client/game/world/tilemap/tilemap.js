...
export class TileSet {
    ...
    async load() {
        ...
        this.setImage = await AssetManager.loadImage(this.name, this.imageSource, true);
...
export class Tile {
    ...
    setImageIndex(imageIndex) {
        ...
        this.image = this.tileSet.images[imageIndex];
        this.imageIndex = imageIndex;
    };
...
export class TileMap {
    ...
    optimizeColliders() {
        ...
        for (let x = 0; x < this.map.length; x++) {
            for (let y = 0; y < this.map[x].length; y++) {
                setupColliders(this.map[x][y], this);
            };
        };
    };

    updateColliders(startTile) {
        startTile.destroyCollider();
        setupColliders(startTile, this);
        let targetTiles = [startTile];
        const UpdatedTiles = {};
        for (let i = 0; i < targetTiles.length; i++) {
            targetTiles[i].destroyCollider();
            setupColliders(targetTiles[i], this);
            UpdatedTiles[targetTiles[i].key] = true;

            const nLeft = targetTiles[i].getNeighbour(-1, 0);
            const nTop = targetTiles[i].getNeighbour(0, -1);
            const nRight = targetTiles[i].getNeighbour(1, 0);
            const nBottom = targetTiles[i].getNeighbour(0, 1);

            let neighbours = [nLeft, nTop, nRight, nBottom];
            for (let j = 0; j < neighbours.length; j++) {
                if (!neighbours[j] || UpdatedTiles[neighbours[j].key]) continue;
                // const oldColliders = {
                //     collider: Boolean(neighbours[j].collider),
                //     leftCollider: Boolean(neighbours[j].leftCollider),
                //     rightCollider: Boolean(neighbours[j].rightCollider),
                //     topCollider: Boolean(neighbours[j].topCollider),
                //     bottomCollider: Boolean(neighbours[j].bottomCollider),
                // };

                UpdatedTiles[neighbours[j].key] = true;

                neighbours[j].destroyCollider();
                setupColliders(neighbours[j], this);
            };
        };
    };
};

const setupColliders = function (tile, tileMap) {
    ...
    if (tile.imageIndex == 0 || tile.imageIndex == 1) {
        ...
        if (!nLeft?.image || nLeft.imageIndex == 3 || nLeft.imageIndex == 4) {
            tile.leftCollider = new Vector(
                tile.x * tileSize + tileMap.room.x - tileMap.room.width / 2,
                tile.y * tileSize + tileMap.room.y - tileMap.room.height / 2,
                tile.x * tileSize + tileMap.room.x - tileMap.room.width / 2,
                tile.y * tileSize + tileMap.room.y - tileMap.room.height / 2 + tileSize,
            );
            tileMap.room.ramps.push(tile.leftCollider);
            tile.leftCollider.tile = tile;
        };
        if (!nRight?.image || nRight.imageIndex == 2 || nRight.imageIndex == 5) {
            tile.rightCollider = new Vector(
                tile.x * tileSize + tileMap.room.x - tileMap.room.width / 2 + tileSize,
                tile.y * tileSize + tileMap.room.y - tileMap.room.height / 2,
                tile.x * tileSize + tileMap.room.x - tileMap.room.width / 2 + tileSize,
                tile.y * tileSize + tileMap.room.y - tileMap.room.height / 2 + tileSize,
            );
            tileMap.room.ramps.push(tile.rightCollider);
        ...
        if (!nTop?.image || nTop.imageIndex == 3 || nTop.imageIndex == 5) {
            ...
            tileMap.room.ramps.push(tile.topCollider);
        ...
        if (!nBottom?.image || nBottom.imageIndex == 2 || nBottom.imageIndex == 4) {
            ...
            tileMap.room.ramps.push(tile.bottomCollider);
        ...
        tileMap.room.ramps.push(tile.collider);
...
export const loadTileSets = async function () {
    startLoading('TILE SETS');
    for (const tileSet of Object.values(TileSets)) {
        await tileSet.load();
    };
    stopLoading('TILE SETS');
};